...
beta.multi(community_Collembola_Groups0.05, index.family="sorensen")

##turnover by pairs, nmds, anosim
beta.pair(community_Collembola_Groups0.05, index.family="sorensen")->beta.pair  ##betadiversity by pair of communities using sorensen on the precense/absence data, with estimation of turnover and nestedness datamatrixes simultaneously
metaMDS (beta.pair$beta.sim)->MDSbetasim_G0.05 ##NMDS

plot (MDSbetasim_G0.05, main="Collembola_Groups_0.05") 
x<- MDSbetasim_G0.05$points[,1]
y<- MDSbetasim_G0.05$points[,2]
text(x, y, pos = 1, cex=0.7, labels = row.names (community_Collembola_Groups0.05))

plot (MDSbetasim_G0.05, main="Collembola_Groups_0.05")
with(general_sample_Mountain2Groups0.05,ordispider(MDSbetasim_G0.05, Mountain2, label=T, col="blue"))

plot (MDSbetasim_G0.05, xlim=c(-0.5, 0.5), ylim=c(-0.4, 0.4), cex.axis=1.4, cex=1.2, cex.lab=1.4, main="Collembola_Groups_0.05")
...
mylabel = bquote(italic(r)^2)
text(x=0.23, y=-0.35, labels = mylabel, cex=1.5)
text(x=0.48, y=-0.35, labels="=0.15 ns", cex=1.5)
mtext(c("lineages 5%"), side = 3, col = "black", line = 1, cex = 2)
