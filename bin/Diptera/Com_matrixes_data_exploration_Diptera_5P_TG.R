...
beta.multi(community_Diptera_Traps0.05, index.family="sorensen")

##turnover by pairs, nmds, anosim
beta.pair(community_Diptera_Traps0.05, index.family="sorensen")->beta.pair  ##betadiversity by pair of communities using sorensen on the precense/absence data, with estimation of turnover and nestedness datamatrixes simultaneously
metaMDS (beta.pair$beta.sim)->MDSbetasim_T0.05 ##NMDS

plot (MDSbetasim_T0.05, main="Diptera_Traps_0.05") 
x<- MDSbetasim_T0.05$points[,1]
y<- MDSbetasim_T0.05$points[,2]
text(x, y, pos = 1, cex=0.7, labels = row.names (community_Diptera_Traps0.05))

plot (MDSbetasim_T0.05, main="Diptera_Traps_0.05")
with(general_sample_Mountain1Traps0.05,ordispider(MDSbetasim_T0.05, Traps, label=T, col="blue"))

plot (MDSbetasim_T0.05, xlim=c(-0.4, 0.4), ylim=c(-0.4, 0.4), cex.axis=1.4, cex=1.2, cex.lab=1.4, main="Diptera_Traps_0.05")
...
text(x=0.48, y=-0.35, labels="=0.139 ns", cex=1.5)
mtext(c("lineage 5%"), side = 3, col = "black", line = 1, cex = 2)
...
dim(community_Diptera0.05)
community_Diptera0.05[which(str_extract (row.names(community_Diptera0.05), "G_") %in% "G_"),]->community_Diptera_Groups0.05
dim(community_Diptera_Groups0.05)
community_Diptera_Groups0.05[,which(colSums(community_Diptera_Groups0.05)!=0)]->community_Diptera_Groups0.05 ##to remove no data colums
dim(community_Diptera_Groups0.05)
write.table (community_Diptera_Groups0.05, file="../../genetic/Data_out/Diptera/Diptera5P/community_Diptera_Groups0.05.txt") ##this is necessary for the format, not able to solve in other way
...
##BY Groups
##**Generating a general table with names and habitat parameters**
row.names(community_Diptera_Groups0.05)->sample_names_Mountain2_0.05
...
general_sample_Mountain2Groups0.05
write.table(general_sample_Mountain2Groups0.05, file="../../genetic/Data_out/Diptera/Diptera5P/general_sample_Mountain2Groups0.05.txt") ##this is the only way I found to be able to work later
...
##**Diptera**
as.matrix(community_Diptera_Groups0.05)->community_Diptera_Groups0.05
row.names(community_Diptera_Groups0.05)->sample_names_Groups0.05
dim(community_Diptera_Groups0.05)->dims_Groups0.05
dims_Groups0.05
.rowSums (community_Diptera_Groups0.05,dims_Groups0.05[1],dims_Groups0.05[2])->sample_richness_Groups0.05 ##summatory by rows
...
##**BY Groups**
write.table(richness_Groups0.05, file="../../genetic/Data_out/Diptera/Diptera5P/richness_Groups0.05_Diptera.txt") ##this is the only way I found to be able to work later
read.table("../../genetic/Data_out/Diptera/Diptera5P/richness_Groups0.05_Diptera.txt",header=TRUE)->richness_Groups0.05

##**BY Groups_C general**
#write.table(richness_GroupsC, file="../../genetic/Data_out/Diptera/Diptera5P/richness_GroupsC_Diptera.txt") ##this is the only way I found to be able to work later
#read.table("../../genetic/Data_out/Diptera/Diptera5P/richness_GroupsC_Diptera.txt",header=TRUE)->richness_GroupsC

##**General plot of richness by sample in Groups**
barplot(richness_Groups0.05$sample_richness_Groups0.05,col=richness_Groups0.05$Mountain2Groups,names.arg= richness_Groups0.05$sample_names_Groups0.05,las=2,cex.names=0.5, ylab="richness_Groups0.05", main="H richness_Groups Diptera_0.05")
...
plot(richness_Groups0.05$Mountain2Groups,richness_Groups0.05$sample_richness_Groups0.05,ylab="richness_Groups_0.05_Diptera", cex=1.4, cex.axis=2.3, lwd=2.5, ylim=c(0,30), col= c("#153a7b", "#eaa22f", "#81b9d0"))
...
##Comparison of each group against. 
text(x=c(1,2,3), y=1.5, labels=c("a","ab","b"), cex=1.5)
...
plot (MDSbetasim_G0.05, main="Diptera_Groups_0.05") 
x<- MDSbetasim_G0.05$points[,1]
y<- MDSbetasim_G0.05$points[,2]
text(x, y, pos = 1, cex=0.7, labels = row.names (community_Diptera_Groups0.05))

plot (MDSbetasim_G0.05, main="Diptera_Groups_0.05")
with(general_sample_Mountain2Groups0.05,ordispider(MDSbetasim_G0.05, Mountain2, label=T, col="blue"))

plot (MDSbetasim_G0.05, xlim=c(-0.5, 0.5), ylim=c(-0.4, 0.4), cex.axis=1.4, cex=1.2, cex.lab=1.4, main="Diptera_Groups_0.05")
...
plot (MDSbetasim_G0.05, xlim=c(-0.4, 0.4), ylim=c(-0.4, 0.4), cex=2, cex.lab=1.4, cex.axis=2.3, lwd=4.5)
...
mylabel = bquote(italic(r)^2)
text(x=0.23, y=-0.35, labels = mylabel, cex=1.5)
text(x=0.48, y=-0.35, labels="=0.29 ns", cex=1.5)
mtext(c("lineage 5%"), side = 3, col = "black", line = 1, cex = 2)
