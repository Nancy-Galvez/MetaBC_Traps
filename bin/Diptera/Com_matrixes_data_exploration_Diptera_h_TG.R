...
#richness_Traps_h_Diptera %>% separate(sample_names_Traps_h, c("Conservation1","Mountain1","Traps","ID"), sep="_",remove=FALSE)->richness_TrapsC
#richness_TrapsC
...
##**BY Traps_C general**
#write.table(richness_TrapsC, file="../../genetic/Data_out/Diptera/Diptera_Haplotypes/richness_TrapsC_Diptera.txt") ##this is the only way I found to be able to work later
#read.table("../../genetic/Data_out/Diptera/Diptera_Haplotypes/richness_TrapsC_Diptera.txt",header=TRUE)->richness_TrapsC
...
beta.multi(community_Diptera_Traps_h, index.family="sorensen")

##turnover by pairs, nmds, anosim
beta.pair(community_Diptera_Traps_h, index.family="sorensen")->beta.pair  ##betadiversity by pair of communities using sorensen on the precense/absence data, with estimation of turnover and nestedness datamatrixes simultaneously
metaMDS (beta.pair$beta.sim)->MDSbetasim_Th ##NMDS

plot (MDSbetasim_Th, main="Diptera_Traps_h") 
x<- MDSbetasim_Th$points[,1]
y<- MDSbetasim_Th$points[,2]
text(x, y, pos = 1, cex=0.7, labels = row.names (community_Diptera_Traps_h))

plot (MDSbetasim_Th, main="Diptera_Traps_h")
with(general_sample_Mountain1Traps_h,ordispider(MDSbetasim_Th, Traps, label=T, col="blue"))

plot (MDSbetasim_Th, xlim=c(-0.4, 0.4), ylim=c(-0.4, 0.4), cex.axis=1.4, cex=1.2, cex.lab=1.4, main="Diptera_Traps_h")
...
text(x=0.50, y=-0.35, labels="=0.096 ns", cex=2)
...
dim(community_Diptera_h)
community_Diptera_h[which(str_extract (row.names(community_Diptera_h), "G_") %in% "G_"),]->community_Diptera_Groups_h
dim(community_Diptera_Groups_h)
community_Diptera_Groups_h[,which(colSums(community_Diptera_Groups_h)!=0)]->community_Diptera_Groups_h ##to remove no data colums
dim(community_Diptera_Groups_h)
write.table (community_Diptera_Groups_h, file="../../genetic/Data_out/Diptera/Diptera_Haplotypes/community_Diptera_Groups_h.txt") ##this is necessary for the format, not able to solve in other way
read.table ("../../genetic/Data_out/Diptera/Diptera_Haplotypes/community_Diptera_Groups_h.txt")->community_Diptera_Groups_h


##**BY Groups**
##**Generating a general table with names and habitat parameters**
row.names(community_Diptera_Groups_h)->sample_names_Mountain2_h
...
general_sample_Mountain2Groups_h
write.table(general_sample_Mountain2Groups_h, file="../../genetic/Data_out/Diptera/Diptera_Haplotypes/general_sample_Mountain2Groups_h.txt") ##this is the only way I found to be able to work later
...
##Diptera
as.matrix(community_Diptera_Groups_h)->community_Diptera_Groups_h
row.names(community_Diptera_Groups_h)->sample_names_Groups_h
dim(community_Diptera_Groups_h)->dims_Groups_h
dims_Groups_h
.rowSums (community_Diptera_Groups_h,dims_Groups_h[1],dims_Groups_h[2])->sample_richness_Groups_h ##summatory by rows
rbind(sample_names_Groups_h,sample_richness_Groups_h)->richness_Groups_h_Diptera
t(richness_Groups_h_Diptera)->richness_Groups_h_Diptera
colnames(richness_Groups_h_Diptera)<-c("sample_names_Groups_h","sample_richness_Groups_h")
richness_Groups_h_Diptera
as.data.frame(richness_Groups_h_Diptera)->richness_Groups_h_Diptera

##**Generating variables with Groups. En la tabla rishnees. Generar variable de montana y sitio.** 
richness_Groups_h_Diptera %>% separate(sample_names_Groups_h, c("Conservation2","Mountain2","Groups","ID"), sep="_",remove=FALSE)->richness_Groups_h_Diptera
richness_Groups_h_Diptera
richness_Groups_h_Diptera %>% unite(Mountain2Groups, Mountain2, Groups, sep="_",remove=FALSE)->richness_Groups_h_Diptera ##generating a variable combining layer and habitat
richness_Groups_h_Diptera

##**Generating variables with in total Groups_C.**
#richness_Groups_h_Diptera %>% separate(sample_names_Groups_h, c("Conservation2","Mountain2","Groups","ID"), sep="_",remove=FALSE)->richness_GroupsC
#richness_GroupsC
...
##**BY Groups**
write.table(richness_Groups_h_Diptera, file="../../genetic/Data_out/Diptera/Diptera_Haplotypes/richness_Groups_h_Diptera.txt") ##this is the only way I found to be able to work later
...
##**BY Groups_C general**
#write.table(richness_GroupsC, file="../../genetic/Data_out/Diptera/Diptera_Haplotypes/richness_GroupsC_Diptera.txt") ##this is the only way I found to be able to work later
#read.table("../../genetic/Data_out/Diptera/Diptera_Haplotypes/richness_GroupsC_Diptera.txt",header=TRUE)->richness_GroupsC

##**General plot of richness by sample in Groups**
barplot(richness_Groups_h_Diptera$sample_richness_Groups_h,col=richness_Groups_h_Diptera$Mountain2Groups,names.arg= richness_Groups_h_Diptera$sample_names_Groups_h, las=2,cex.names=0.5, ylab="richness_Groups_h_Diptera", main="H richness_Groups Diptera")
richness_Groups_h_Diptera %>% group_by(Mountain2Groups) %>% summarise(mean(sample_richness_Groups_h))

##**min, max, ds Summarise**
richness_Groups_h_Diptera %>% group_by(Mountain2Groups) %>% summarise(mean(sample_richness_Groups_h))
richness_Groups_h_Diptera %>% group_by(Mountain2Groups) %>% summarise(min(sample_richness_Groups_h))
richness_Groups_h_Diptera %>% group_by(Mountain2Groups) %>% summarise(max(sample_richness_Groups_h))
richness_Groups_h_Diptera %>% group_by(Mountain2Groups) %>% summarise(sd(sample_richness_Groups_h))
...
plot(richness_Groups_h_Diptera$Mountain2Groups,richness_Groups_h_Diptera$sample_richness_Groups_h,ylab="richness_Groups_h_Diptera", cex=1.4, cex.axis=2.3, lwd=2.5, ylim=c(0,60), col= c("#153a7b", "#eaa22f", "#81b9d0"))
kruskal.test(sample_richness_Groups_h ~ Mountain2Groups, data = richness_Groups_h_Diptera)
#PMCMRplus::kwAllPairsNemenyiTest(x=richness_Groups_h_Diptera$sample_richness_Groups_h, g=richness_Groups_h_Diptera$Mountain2Groups, p.adjust.method = "single-step")
PMCMRplus::kwAllPairsConoverTest(x=richness_Groups_h_Diptera$sample_richness_Groups_h, g=richness_Groups_h_Diptera$Mountain2Groups, p.adjust.method = "single-step")
#PMCMRplus::kwAllPairsDunnTest(x=richness_Groups_h_Diptera$sample_richness_Groups_h, g=richness_Groups_h_Diptera$Mountain2Groups, p.adjust.method="bonferroni")
#kwAllPairsNemenyiTest(x=richness_Groups_h_Diptera$sample_richness_Groups_h, g=richness_Groups_h_Diptera$Mountain2Groups)
##Comparison of each group against. 
#text(x=c(1,2,3,4,5), y=2, labels=c("a","ab","ab","b","ab"), cex=1.5)
text(x=3.5, y=2, labels="ns", cex=2)
...
plot (MDSbetasim_Gh, main="Diptera_Groups_h") 
x<- MDSbetasim_Gh$points[,1]
y<- MDSbetasim_Gh$points[,2]
text(x, y, pos = 1, cex=0.7, labels = row.names (community_Diptera_Groups_h))

plot (MDSbetasim_Gh, main="Diptera_Groups_h")
with(general_sample_Mountain2Groups_h,ordispider(MDSbetasim_Gh, Mountain2, label=T, col="blue"))

plot (MDSbetasim_Gh, xlim=c(-0.5, 0.5), ylim=c(-0.4, 0.4), cex.axis=1.4, cex=1.2, cex.lab=1.4, main="Diptera_Groups_h")
...
mylabel = bquote(italic(r)^2)
text(x=0.25, y=-0.35, labels = mylabel, cex=2)
text(x=0.5, y=-0.35, labels="=0.129 ns", cex=2)
mtext(c("Haplotypes"), side = 3, col = "black", line = 1, cex = 2)
