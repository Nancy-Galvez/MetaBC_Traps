...
beta.multi(community_Diptera_Traps0.03, index.family="sorensen")

##turnover by pairs, nmds, anosim
beta.pair(community_Diptera_Traps0.03, index.family="sorensen")->beta.pair  ##betadiversity by pair of communities using sorensen on the precense/absence data, with estimation of turnover and nestedness datamatrixes simultaneously
metaMDS (beta.pair$beta.sim)->MDSbetasim_T0.03 ##NMDS

plot (MDSbetasim_T0.03, main="Diptera_Traps_0.03") 
x<- MDSbetasim_T0.03$points[,1]
y<- MDSbetasim_T0.03$points[,2]
text(x, y, pos = 1, cex=0.7, labels = row.names (community_Diptera_Traps0.03))

plot (MDSbetasim_T0.03, main="Diptera_Traps_0.03")
with(general_sample_Mountain1Traps0.03,ordispider(MDSbetasim_T0.03, Traps, label=T, col="blue"))

plot (MDSbetasim_T0.03, xlim=c(-0.4, 0.4), ylim=c(-0.4, 0.4), cex.axis=1.4, cex=1.2, cex.lab=1.4, main="Diptera_Traps_0.03")
...
text(x=0.41, y=-0.35, labels="=0.02 ns", cex=2)
mtext(c("lineage 3%"), side = 3, col = "black", line = 1, cex = 2)
...
dim(community_Diptera0.03)
community_Diptera0.03[which(str_extract (row.names(community_Diptera0.03), "G_") %in% "G_"),]->community_Diptera_Groups0.03
dim(community_Diptera_Groups0.03)
community_Diptera_Groups0.03[,which(colSums(community_Diptera_Groups0.03)!=0)]->community_Diptera_Groups0.03 ##to remove no data colums
dim(community_Diptera_Groups0.03)
write.table (community_Diptera_Groups0.03, file="../../genetic/Data_out/Diptera/Diptera3P/community_Diptera_Groups0.03.txt") ##this is necessary for the format, not able to solve in other way
...
##BY Groups
##**Generating a general table with names and habitat parameters**
row.names(community_Diptera_Groups0.03)->sample_names_Mountain2_0.03
...
general_sample_Mountain2Groups0.03
write.table(general_sample_Mountain2Groups0.03, file="../../genetic/Data_out/Diptera/Diptera3P/general_sample_Mountain2Groups0.03.txt") ##this is the only way I found to be able to work later
...
##**Diptera**
as.matrix(community_Diptera_Groups0.03)->community_Diptera_Groups0.03
row.names(community_Diptera_Groups0.03)->sample_names_Groups0.03
dim(community_Diptera_Groups0.03)->dims_Groups0.03
dims_Groups0.03
.rowSums (community_Diptera_Groups0.03,dims_Groups0.03[1],dims_Groups0.03[2])->sample_richness_Groups0.03 ##summatory by rows
...
##**BY Groups**
write.table(richness_Groups0.03, file="../../genetic/Data_out/Diptera/Diptera3P/richness_Groups0.03_Diptera.txt") ##this is the only way I found to be able to work later
read.table("../../genetic/Data_out/Diptera/Diptera3P/richness_Groups0.03_Diptera.txt",header=TRUE)->richness_Groups0.03

##**BY Groups_C general**
#write.table(richness_GroupsC, file="../../genetic/Data_out/Diptera/Diptera3P/richness_GroupsC_Diptera.txt") ##this is the only way I found to be able to work later
#read.table("../../genetic/Data_out/Diptera/Diptera3P/richness_GroupsC_Diptera.txt",header=TRUE)->richness_GroupsC

##**General plot of richness by sample in Groups**
barplot(richness_Groups0.03$sample_richness_Groups0.03,col=richness_Groups0.03$Mountain2Groups,names.arg= richness_Groups0.03$sample_names_Groups0.03,las=2,cex.names=0.5, ylab="richness_Groups0.03", main="H richness_Groups Diptera_0.03")
...
plot(richness_Groups0.03$Mountain2Groups,richness_Groups0.03$sample_richness_Groups0.03,ylab="richness_Groups_0.03_Diptera", cex=1.4, cex.axis=2.3, lwd=2.5, ylim=c(0,30), col= c("#153a7b", "#eaa22f", "#81b9d0"))
...
##Comparison of each group against. 
text(x=c(1,2,3), y=1.4, labels=c("a","ab","b"), cex=1.5)
text(x=3.5, y=2, labels="*", cex=2)
...
plot (MDSbetasim_G0.03, main="Diptera_Groups_0.03") 
x<- MDSbetasim_G0.03$points[,1]
y<- MDSbetasim_G0.03$points[,2]
text(x, y, pos = 1, cex=0.7, labels = row.names (community_Diptera_Groups0.03))

plot (MDSbetasim_G0.03, main="Diptera_Groups_0.03")
with(general_sample_Mountain2Groups0.03,ordispider(MDSbetasim_G0.03, Mountain2, label=T, col="blue"))

plot (MDSbetasim_G0.03, xlim=c(-0.5, 0.5), ylim=c(-0.4, 0.4), cex.axis=1.4, cex=1.2, cex.lab=1.4, main="Diptera_Groups_0.03")
...
text(x=0.5, y=-0.37, labels="=0.24 ns", cex=1.5)
mtext(c("lineages 3%"), side = 3, col = "black", line = 1, cex = 2)
