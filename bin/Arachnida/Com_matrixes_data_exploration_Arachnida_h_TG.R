...
beta.multi(community_Arachnida_Traps_h, index.family="sorensen")

##turnover by pairs, nmds, anosim
beta.pair(community_Arachnida_Traps_h, index.family="sorensen")->beta.pair  ##betadiversity by pair of communities using sorensen on the precense/absence data, with estimation of turnover and nestedness datamatrixes simultaneously
metaMDS (beta.pair$beta.sim)->MDSbetasim_Th ##NMDS

plot (MDSbetasim_Th, main="Arachnida_Traps_h") 
x<- MDSbetasim_Th$points[,1]
y<- MDSbetasim_Th$points[,2]
text(x, y, pos = 1, cex=0.7, labels = row.names (community_Arachnida_Traps_h))
...
dim(community_Arachnida_h)
community_Arachnida_h[which(str_extract (row.names(community_Arachnida_h), "G_") %in% "G_"),]->community_Arachnida_Groups_h
dim(community_Arachnida_Groups_h)
community_Arachnida_Groups_h[,which(colSums(community_Arachnida_Groups_h)!=0)]->community_Arachnida_Groups_h ##to remove no data colums
dim(community_Arachnida_Groups_h)
write.table (community_Arachnida_Groups_h, file="../../genetic/Data_out/Arachnida/Arachnida_Haplotypes/community_Arachnida_Groups_h.txt") ##this is necessary for the format, not able to solve in other way
read.table ("../../genetic/Data_out/Arachnida/Arachnida_Haplotypes/community_Arachnida_Groups_h.txt")->community_Arachnida_Groups_h


##**BY Groups**
##**Generating a general table with names and habitat parameters**
row.names(community_Arachnida_Groups_h)->sample_names_Mountain2_h
...
general_sample_Mountain2Groups_h
write.table(general_sample_Mountain2Groups_h, file="../../genetic/Data_out/Arachnida/Arachnida_Haplotypes/general_sample_Mountain2Groups_h.txt") ##this is the only way I found to be able to work later
read.table("../../genetic/Data_out/Arachnida/Arachnida_Haplotypes/general_sample_Mountain2Groups_h.txt",header=TRUE)->general_sample_Mountain2Groups_h


#**HAPLOTYPE RICHNESS TABLES, PLOTS AND ANALYSES by GroupsS** 
##Arachnida
as.matrix(community_Arachnida_Groups_h)->community_Arachnida_Groups_h
row.names(community_Arachnida_Groups_h)->sample_names_Groups_h
dim(community_Arachnida_Groups_h)->dims_Groups_h
dims_Groups_h
.rowSums (community_Arachnida_Groups_h,dims_Groups_h[1],dims_Groups_h[2])->sample_richness_Groups_h ##summatory by rows
rbind(sample_names_Groups_h,sample_richness_Groups_h)->richness_Groups_h_Arachnida
t(richness_Groups_h_Arachnida)->richness_Groups_h_Arachnida
colnames(richness_Groups_h_Arachnida)<-c("sample_names_Groups_h","sample_richness_Groups_h")
richness_Groups_h_Arachnida
as.data.frame(richness_Groups_h_Arachnida)->richness_Groups_h_Arachnida

##**Generating variables with Groups. En la tabla rishnees. Generar variable de montana y sitio.** 
richness_Groups_h_Arachnida %>% separate(sample_names_Groups_h, c("Conservation2","Mountain2","Groups","ID"), sep="_",remove=FALSE)->richness_Groups_h_Arachnida
richness_Groups_h_Arachnida
richness_Groups_h_Arachnida %>% unite(Mountain2Groups, Mountain2, Groups, sep="_",remove=FALSE)->richness_Groups_h_Arachnida ##generating a variable combining layer and habitat
richness_Groups_h_Arachnida

##**Generating variables with in total Groups_C.**
#richness_Groups_h_Arachnida %>% separate(sample_names_Groups_h, c("Conservation2","Mountain2","Groups","ID"), sep="_",remove=FALSE)->richness_GroupsC
#richness_GroupsC
...
##**BY Groups**
write.table(richness_Groups_h_Arachnida, file="../../genetic/Data_out/Arachnida/Arachnida_Haplotypes/richness_Groups_h_Arachnida.txt") ##this is the only way I found to be able to work later
read.table("../../genetic/Data_out/Arachnida/Arachnida_Haplotypes/richness_Groups_h_Arachnida.txt",header=TRUE)->richness_Groups_h_Arachnida

##**BY Groups_C general**
#write.table(richness_GroupsC, file="../../genetic/Data_out/Arachnida/Arachnida_Haplotypes/richness_GroupsC_Arachnida.txt") ##this is the only way I found to be able to work later
#read.table("../../genetic/Data_out/Arachnida/Arachnida_Haplotypes/richness_GroupsC_Arachnida.txt",header=TRUE)->richness_GroupsC

##**General plot of richness by sample in Groups**
barplot(richness_Groups_h_Arachnida$sample_richness_Groups_h,col=richness_Groups_h_Arachnida$Mountain2Groups,names.arg= richness_Groups_h_Arachnida$sample_names_Groups_h, las=2,cex.names=0.5, ylab="richness_Groups_h_Arachnida", main="H richness_Groups Arachnida")
richness_Groups_h_Arachnida %>% group_by(Mountain2Groups) %>% summarise(mean(sample_richness_Groups_h))

##**min, max, ds Summarise**
richness_Groups_h_Arachnida %>% group_by(Mountain2Groups) %>% summarise(mean(sample_richness_Groups_h))
richness_Groups_h_Arachnida %>% group_by(Mountain2Groups) %>% summarise(min(sample_richness_Groups_h))
richness_Groups_h_Arachnida %>% group_by(Mountain2Groups) %>% summarise(max(sample_richness_Groups_h))
richness_Groups_h_Arachnida %>% group_by(Mountain2Groups) %>% summarise(sd(sample_richness_Groups_h))
...
plot(richness_Groups_h_Arachnida$Mountain2Groups,richness_Groups_h_Arachnida$sample_richness_Groups_h,ylab="richness_Groups_h_Arachnida", cex=1.4, cex.axis=2.3, lwd=2.5, ylim=c(0,20))
kruskal.test(sample_richness_Groups_h ~ Mountain2Groups, data = richness_Groups_h_Arachnida)
#PMCMRplus::kwAllPairsNemenyiTest(x=richness_Groups_h_Arachnida$sample_richness_Groups_h, g=richness_Groups_h_Arachnida$Mountain2Groups, p.adjust.method = "single-step")
PMCMRplus::kwAllPairsConoverTest(x=richness_Groups_h_Arachnida$sample_richness_Groups_h, g=richness_Groups_h_Arachnida$Mountain2Groups, p.adjust.method = "single-step")
#PMCMRplus::kwAllPairsDunnTest(x=richness_Groups_h_Arachnida$sample_richness_Groups_h, g=richness_Groups_h_Arachnida$Mountain2Groups, p.adjust.method="bonferroni")
#kwAllPairsNemenyiTest(x=richness_Groups_h_Arachnida$sample_richness_Groups_h, g=richness_Groups_h_Arachnida$Mountain2Groups)
##Comparison of each group against. 
...
beta.multi(community_Arachnida_Groups_h, index.family="sorensen")

##turnover by pairs, nmds, anosim
beta.pair(community_Arachnida_Groups_h, index.family="sorensen")->beta.pair  ##betadiversity by pair of communities using sorensen on the precense/absence data, with estimation of turnover and nestedness datamatrixes simultaneously
metaMDS (beta.pair$beta.sim)->MDSbetasim_Gh ##NMDS

plot (MDSbetasim_Gh, main="Arachnida_Groups_h") 
x<- MDSbetasim_Gh$points[,1]
y<- MDSbetasim_Gh$points[,2]
text(x, y, pos = 1, cex=0.7, labels = row.names (community_Arachnida_Groups_h))

plot (MDSbetasim_Gh, main="Arachnida_Groups_h")
with(general_sample_Mountain2Groups_h,ordispider(MDSbetasim_Gh, Mountain2, label=T, col="blue"))

plot (MDSbetasim_Gh, xlim=c(-0.5, 0.5), ylim=c(-0.4, 0.4), cex.axis=1.4, cex=1.2, cex.lab=1.4, main="Arachnida_Groups_h")
...
text(x=0.45, y=-0.35, labels="=0.07 ns", cex=2)
mtext(c("Haplotypes"), side = 3, col = "black", line = 1, cex = 2)
