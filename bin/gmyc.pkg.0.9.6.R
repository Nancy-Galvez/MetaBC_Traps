...
summary.gmyc <- function(res) {
	...
	cat("\n\tmaximum likelihood of GMYC model:\t", max(res$likelihood), sep="")
	
	#chisq test
	LR <- 2*(max(res$likelihood)-res$likelihood[1])
...
gmyc <- function(tr, method = "single", interval = c(0, 5), quiet = FALSE) {
	...
	gmyc.likelihood <- function() {	
		...
		l.results <- rep(NA, 7)
		#x<-optim(c(1, 1),l.min2,control=list(fnscale=-1))	#likelihood for GMYC model
		
		x <- optim(c(1, 1), l.min2, control=list(fnscale=-1), lower=min(interval), upper=max(interval))
	...
	gmyc.single <- function() {
		...
		ml <- 0
		if (!quiet) { print(c("node", "T", "loglik"), quote=F) }
		...
		for (j in (stthresh:nthresh)) {
			...
			mrca[2:numnod]<- (bt[as.integer(tr$edge[,1][tr$edge[,2]>numtip]) - numtip]<threshy)&(bt[as.integer(tr$edge[,2][tr$edge[,2]>numtip]) - numtip]>=threshy)	##???
					
			nod.type <- (bt>=threshy)+mrca
			...
			if (!quiet) { print (c(j, threshy, l.results[j,1])) }
	...
	gmyc.multi <- function() {
		...
		##search for 3 part 11/09/08
		##revised 15/09/08
		select.start0 <- function(time, ml) {	#recurive division of starting point of d-f algoritm
			result <- c()
			result.mrca <- list()
			...
			for (i in 1:length(improve)) {
				if (improve[i] && length(part[[i]]) > 2) {
					cat("recursively try part of", start[i], "\n") 
					temp <- select.start(part[[i]], max(result[,1]))
					result <- rbind(result, temp[[1]])
					result.mrca <- c(result.mrca, temp[[2]])
				}
			}
	...
	if (method == "single") {
		...
		stop("Invalid name of optimiztion method. Only single or multiple are available.")
	}
